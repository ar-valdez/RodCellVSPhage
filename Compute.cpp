...
void mean_stress(const Cell& cell, const Cell* cell_array, const int* neighbours, \
                 const int caseTest, UniformGrid& Grid, DoubleArray2D& Wall, DoubleArray2D& Height, \
                 CoordArray2D& Normal, Tensor& stressTensor, DoubleCoord& Fnet)
{
	int ID;
	DoubleCoord F, r, F2, r2, vn; // gives force, position of force
	...
	for (int neighbourID = 1; neighbourID<numNeighbours+1; neighbourID++){
		...
		F_cc(cell, cell_array[ID], caseTest, F, r, d, vn);
		
		if(isnan(F.x + F.y + F.z) == false)
		{
...
void GetDensity(DoubleArray3D& DensityVec, DoubleArray3D& Density1Vec, \
                DoubleArray3D& Density2Vec, DoubleArray3D& insideColonyDen, \
                DoubleArray2D& Height, UniformGrid& Grid, Cell* cells, \
                int& minx, int& maxx, int& miny, int& maxy, int& maxz)
{
			...
			for (int boxz = 0; boxz < DensityVec.Size().z; boxz++)
			{
				...
				for (int celli = 0; celli < count; celli++)
				{
					...
					switch(cell.Type)
					{
						...
						default:
							//std::cout << "CellType error!" << std::endl;
							//exit(0);
							break;
					}
...
void GetHeight(DoubleArray3D& DensityVec, DoubleArray2D& Height, UniformGrid& Grid, \
               Cell* cells, int& minx, int& maxx, int& miny, int& maxy, int& maxz)
{
...
void ShiftDensity(DoubleArray3D& DensityVec, DoubleArray3D& Density1Vec, \
                  DoubleArray3D& Density2Vec, DoubleArray2D& DensityWallVec, \
                  DoubleArray2D& DensityWallVec1, DoubleArray2D& DensityWallVec2, \
                  DoubleArray3D& DensityVecShiftP, DoubleArray3D& Density1VecShiftP, \
                  DoubleArray3D& Density2VecShiftP,DoubleArray2D& DensityWallVecShiftP, \
                  DoubleArray2D& DensityWallVec1ShiftP, DoubleArray2D& DensityWallVec2ShiftP, \
...
void BoxAverage(DoubleArray3D& RoughDensity, DoubleArray3D& Density, \
                DoubleArray2D& RoughWallDensity, DoubleArray2D& WallDensity, \
                DoubleArray3D& Filter, int FilterDim, int minx, int maxx, \
                int miny, int maxy, int maxz, int BoxX, int BoxY, int BoxZ)
{
...
// smooths any array
void Smooth(DoubleArray2D& InArray, DoubleArray2D& OutArray, DoubleArray2D& Filter, \
            int FilterDim, int minx, int maxx, int miny, int maxy)
{
	// need smaller basis for edges
	double avg, h;
