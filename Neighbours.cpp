...
// They key is removed from the 
void removeKeyNeighbours(int* localNeighbourList, int ID_length, int key)
{
        for(int k=1; k<ID_length; k++)
        {
                if(localNeighbourList[k] == key)
                {
                        int temp = localNeighbourList[ID_length];       // last entry
                        localNeighbourList[k] = temp;                   // bring last entry to key place
                        localNeighbourList[0] = ID_length - 1;          // Reduce in one the length
                        break;
                }
        }
}
