...
private:
	...
	struct Box
	{
		...
		void Add(int cellID)
		{
		        ...
		        {
        			//std::cout << "Tried to add too many cells" << std::endl;
		...
		void Remove(int cellID)
		{
			...
			{
			        //std::cout << "Conflict cell: " << cellID << std::endl;
...
public:
	...
	Address GetAddress(DoubleCoord position)
	{
		...
		{
                        std::cout << "x,y,z, index: " << x_index << " " <<  y_index << " " << z_index << " " \
                                  << "; x,y,z, sizes: " << m_Size.x << " " <<  m_Size.y << " " << m_Size.z << " " \
                                  << "; x,y,z, pos: "   << position.x << " " <<  position.y << " " << position.z << " " \
                                  << "; Grid size: "    << m_BoxLength << std::endl;
                        std::cout << "Cells have expanded farther than boxes allows" << std::endl;
                        //exit(0);
		}
				...
				for (int xa = max(a0.x-1,0); xa<min(a0.x+2,m_Size.x); xa++)
				{
					...
					for (int cella = 0; cella<box->Count; cella++)
					{
						...
						if (neighbourID!=cellID)
						{
							...
							if(neighboursFound > maxNeighbours)
							{
	...
	void GetCellsinBox(int x_index, int y_index, int z_index, int* CellinBox)
	{
	        Box* box  = m_Boxes[Index(x_index, y_index, z_index)];
	        int count = box->Count;
	        for(int k=0; k< count; k++)
	        {
	                CellinBox[k] = box->Cells[k];
	        }
	}
